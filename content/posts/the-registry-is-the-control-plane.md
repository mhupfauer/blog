...
Concrete shape. An assistant has a web-search tool that reaches the public internet and a pull-internal-data tool that retrieves classified material from an internal store. The naive framing is that one ordering is safe and the other is dangerous — but that framing is wrong in both directions. Web-before-internal is not automatically safe, because untrusted public content reaching the model first creates an indirect prompt-injection path into the privileged internal query. Internal-before-web is not automatically dangerous, because the question is whether classified values can flow into the egress call, not whether the calls were issued in a particular order. The control is not a fixed tool sequence. It is value provenance plus egress policy.
...
Above that sits the contained agent session — Microsoft's docs call it that; the user-facing name is Agent Workspace. It is not a Hyper-V VM and not Windows Sandbox. It is a separate Windows logon session under a low-privilege agent user account with its own files and its own registry hive. The isolation guarantees are the boring kind: separate token, separate ACLs, no default access to the user's profile, settings, credentials, or running app windows; network egress open by default; file access into a handful of user folders only when the host app's MSIX declares the capability and the user consents at prompt time. Intune sits on top, with admin-controls documentation still partial in preview.
